...
const ProfilePage = () => {
  ...
  useEffect(() => {
    ...
    fetch('/api/getproducts')
      .then((res) => res.json())
      .then((data) => {
        const purchases = data.filter((p) => purchasedIds.includes(p.product_id));
        setPurchasedProducts(purchases);
      });
  }, []);
  ...
  const isPasswordStrong = (pwd) =>
    pwd.length >= 5 && /[A-Z]/.test(pwd) && /[0-9]/.test(pwd) && /[^A-Za-z0-9]/.test(pwd);
  ...
  return (
    <div className="hero-banner">
      <div className="container-fluid btn btn-dark">
        <div className="profile-wrapper p-4" style={{ fontFamily: 'sans-serif' }}>
          {/* Header */}
          <div className="d-flex justify-content-between align-items-center btn btn-secondary mx-2">
            <h2>{user.username}'s Profile</h2>
            <button
          ...
          <div className="d-flex mt-3">
            ...
            <div className="flex-3 ps-4">
              ...
              {menuOpen && (
                <div className="btn btn-secondary mt-3 p-3">
                  <input
                    type="text"
                    name="username"
                    value={editFields.username}
                  ...
                  <div className="mt-2">
                    <button
                      ...
                      onClick={() =>
                        setEditFields({ ...editFields, password: generateStrongPassword() })
                      }
                    >
                      Auto Generate Password
                    </button>
          ...
          {/* Purchased Products */}
          <h3 className="mt-4">Your Purchased Products</h3>
          {purchasedProducts.length > 0 ? (
              ...
              >
                {purchasedProducts.map((p) => (
                  ...
                  >
                    ...
                    <p>{p.product_desc}</p>
                    <p>
                      <strong>Status:</strong> Purchased
                    </p>
                  </div>
                ))}
              </div>
